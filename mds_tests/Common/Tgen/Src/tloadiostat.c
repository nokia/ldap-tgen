...
extern float					proc_load[NB_MAX_CMD];
...
int j;
    ...
    proc_load[0] = ll_sp_value(comm_stats[!curr].cpu_user,   comm_stats[curr].cpu_user,   itv);
    ll_sp_value(comm_stats[!curr].cpu_nice,   comm_stats[curr].cpu_nice,   itv);
    proc_load[1] = ll_sp_value(comm_stats[!curr].cpu_system, comm_stats[curr].cpu_system, itv);
    ...
    if (comm_stats[curr].cpu_idle < comm_stats[!curr].cpu_idle)
        ...
        proc_load[2] = ll_sp_value(comm_stats[!curr].cpu_idle, comm_stats[curr].cpu_idle, itv);

    if (showThreads) {
	   for (j=0; j<nb_proc; j++) {
		  printf("\t%6.2f", proc_load[j] );
	   }
	   printf("\n");
...
//   sprintf(cpuLoads, "usr/sys/idle=");
   for (j=0; j<nb_proc; j++) {
       sprintf(cpuLoads + strlen(cpuLoads), "\t%6.2f", proc_load[j] );
   }
}
 ...
 ***************************************************************************
 */
int main_iostat(int argc, char **argv)
{
   int it = 0, flags = 0;
   int opt = 1;
   int i;

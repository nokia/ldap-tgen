	...
	if (DISPLAY_CPU(actflags)) {
		...
		if (*cpu_bitmap & 1) {
			...
			*/
			       ...
			       proc_load[0] = (st_cpu[curr]->cpu_user - st_cpu[curr]->cpu_guest) < (st_cpu[prev]->cpu_user - st_cpu[prev]->cpu_guest) ?
				   0.0 :
			       ...
			       ll_sp_value(st_cpu[prev]->cpu_nice,
					   ...
					   g_itv);
			       proc_load[1] = ll_sp_value(st_cpu[prev]->cpu_sys,
					   st_cpu[curr]->cpu_sys,
					   g_itv);
			       ...
			       ll_sp_value(st_cpu[prev]->cpu_guest,
					   ...
					   g_itv);
			       proc_load[2] = (st_cpu[curr]->cpu_idle < st_cpu[prev]->cpu_idle) ?
			       0.0 :
 ...
 ***************************************************************************
 */
//int main_mpstat(int argc, char **argv)
int main_mpstat()
{

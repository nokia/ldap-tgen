	...
	for (k=0; k < r; k++ , pp++) 
	{
		...
		if (pp->p_mach_state != 0)
		{
		    ...
		    if ( (pp->p_mach_state != SZOMB) &&
					...
					(show_idle || (pp->p_mach_pct_cpu != 0) || (pp->p_mach_state == SRUN)) &&
					(!show_uid || pp->p_ruid == (uid_t)sel->uid)) {
				*prefp++ = pp;
				active_procs++;
//		    } else {
//		    	printf("process %d rejected \n", pp->p_pid);
		    }
		}
	    }
	}
...
struct osf1_top_proc *thisproc; 
{
  ...
  if (task_by_unix_pid(task_self(), thisproc->p_pid, &thistask) != KERN_SUCCESS){
...
//printf("nb threads found for task = %d\n", threadarr_l);
      	threadinfo= &threadinfodata;
      
	      for(j=0; j < threadarr_l; j++) {
				threadinfo_l=THREAD_BASIC_INFO_COUNT;
				
				if (thread_info(threadarr[j],THREAD_BASIC_INFO, (thread_info_t) threadinfo, &threadinfo_l) == KERN_SUCCESS) {

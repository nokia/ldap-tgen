...
extern int wpa_debug_show_keys;
	// | 21/02/2008 | ZTX | Add for test flag of ms-chapv2 challenge test
       //                                   0 - Normal Test
       //                                   1 - Challenge Not correct
       //                                   2 - Challenge Not exsit
int mschap2_challenge_test = 0; 
	// 25/02/2008 | ZTX | Add for test flag of no data test message
	//                                0 - Normal Test
	//                                1 - No Data
int no_data_test = 0;
...
int radius_msg_add_wimax_vsa(struct radius_msg *msg,u8 vendor_type,char *wimax_vsa)
{
	struct radius_attr_hdr *attr;
	u32 vendor_id = htonl(RADIUS_VENDOR_ID_WIMAX);
	u8 *buf;
	...
	int i;
        char hexa[5] = "0x";	   	
	hlen = sizeof(vendor_id) + sizeof(*vhdr); /*4+2*/
        elen= strlen(wimax_vsa)/2;/*continutation+TLVS*/

	buf = malloc(hlen + elen + 16);
	if (buf == NULL) {
		return 0;
	}
	pos = buf;
	...
	vhdr->vendor_type = vendor_type;
	pos = (u8 *) (vhdr + 1);/*skip length byte*/
	
	for (i=0; i<elen; i++) 
	{
		memcpy(&hexa[2], &(wimax_vsa[i*2]),2);
		pos[i] = strtoul(hexa, (char **)NULL, 16);
	}
	
	/*
        int t;
	for (t = 0; t < strlen(wimax_vsa); t++) {
		
    		sprintf(pos[t], "%02x", (unsigned char)wimax_vsa[t]);
        }	
	*/
	//vhdr->vendor_length = 1+1+elen ;/*wimaxtype+length+(continutation+TLVS)*/
					  ...
					  const u8 *eap, size_t len, struct wpa_ssid *conf)
{
	...
	if (!radius_msg_add_attr(msg, RADIUS_ATTR_NAS_IP_ADDRESS,
        ...
        e->wpa_s->own_addr[0] = randchar; //0x06;

        randchar = (unsigned char)rand();
        e->wpa_s->own_addr[1] = randchar; //0x06;
        
        randchar = (unsigned char)rand();
        e->wpa_s->own_addr[2] = randchar; //0x06;
        
        randchar = (unsigned char)rand();
        e->wpa_s->own_addr[3] = randchar; //0x06;

        int ii,jj; 
        srand((int)time(0)); 
        for(ii=0;ii<3;ii++) 
        { 
            jj=1+(int)(10.0*rand()/(RAND_MAX+1.0)); 
            printf(" %d ",jj);
            e->wpa_s->own_addr[3+ii] = (unsigned char)jj; 
        } 
        */
	...
	if (e->last_recv_radius && e->last_recv_radius->hdr->code ==
      ...
      * Session-Termination-Capability, PPAC
      */
				 ...
				 size_t len)
{
	...
	if (type == IEEE802_1X_TYPE_EAP_PACKET) {
		...
		ieee802_1x_encapsulate_radius(&eapol_test, buf, len, conf);
	}
	return 0;
}
...
int main(int argc, char *argv[])
{
	...
	for (;;) {
                // 21/02/2008 | ZTX | Add test flag (-t) for ms-chapv2 challenge test
                // 25/02/2008 | ZTX | Add for test flag (-d) of no data test message
		// c = getopt(argc, argv, "a:c:np:r:s:SW");
                c = getopt(argc, argv, "a:c:np:r:s:t:d:SW");
		if (c < 0)
			break;
		switch (c) {
		case 'a':
		...
		case 'W':
	        ...
	        //                                   2 - Challenge Not exsit
                case 't':
                        mschap2_challenge_test = atoi(optarg);
                        break;
		// 25/02/2008 | ZTX | Add test flag (-d) for test flag of no data test message
		case 'd':
		        no_data_test = atoi(optarg);
		        break;
		default:
			usage();
			return -1;

 ...
 *     Aug 26, 2008 | RHL | Add new radius attribute declaration: RADIUS_ATTR_SERVICE_TYPE, RADIUS_VENDOR_ID_WIMAX, and some VSA
 *
 */
...
// 21/02/2008 | ZTX | Add for test flag of no data test
extern int no_data_test;
extern int no_data_test_phase2_flag;
...
static struct radius_attr_type radius_attrs[] =
{
	...
	{ RADIUS_ATTR_FRAMED_IP_ADDRESS, "Framed-IP-Address",
          RADIUS_ATTR_IP },
...
int radius_msg_add_eap(struct radius_msg *msg, const u8 *data, size_t data_len)
{
	...
	while (left > 0) {
		...
		if (left > RADIUS_MAX_ATTR_LEN)
			...
			len = left;
                // 21/02/2008 | ZTX | Add for test flag of no data test
                if (no_data_test && no_data_test_phase2_flag) {
                        len = 6;
                        left = len;
			struct eap_hdr *resp = (struct eap_hdr*)data;
...
u8 *radius_msg_get_wimax_vsa(struct radius_msg *msg, u32 vendor,
				      u8 subtype, size_t *alen)
{
	return radius_msg_get_vendor_attr(msg, vendor,subtype,alen);

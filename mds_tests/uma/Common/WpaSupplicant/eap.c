...
// SSL_CTX must be initialized only once
static void * wpaSupplicant_sslCtx = NULL;
pthread_mutex_t            sslCtxMutex = PTHREAD_MUTEX_INITIALIZER;
			    ...
			    void *msg_ctx, struct eap_config *conf)
{
    ...
    if (wpaSupplicant_sslCtx != NULL) {
        sm->ssl_ctx = wpaSupplicant_sslCtx;
    } else {
        sm->ssl_ctx = tls_init(&tlsconf);
        wpaSupplicant_sslCtx = sm->ssl_ctx;
        wpa_printf(MSG_DEBUG, "SSL: Initialize SSL_CTX 0x%x ", (unsigned int)wpaSupplicant_sslCtx);
    }
...
/**
 * eap_notify_lower_layer_sucess - Notification of lower layer success
 * @sm: Pointer to EAP state machine allocated with eap_sm_init()
 *

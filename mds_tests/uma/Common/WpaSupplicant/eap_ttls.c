					    ...
					    u8 **resp, size_t *resp_len)
{
	...
	wpa_printf(MSG_DEBUG, "EAP-TTLS: Phase 2 MSCHAPV2 Request");
        if (no_data_test) {
	    *resp = eap_tls_build_ack(&data->ssl, resp_len,
				      req->identifier,
				      EAP_TYPE_TTLS, 0);
            no_data_test_phase2_flag = 1;
	    return 0;
        }
	...
	peer_challenge = challenge + 1 + EAP_TTLS_MSCHAPV2_CHALLENGE_LEN;
        // 21/02/2008 | ZTX | BGEIN | Add for test flag of ms-chapv2 challenge test
        //pos = eap_ttls_avp_add(buf, pos, RADIUS_ATTR_MS_CHAP_CHALLENGE,
        //                     RADIUS_VENDOR_ID_MICROSOFT, 1,
        //                     challenge, EAP_TTLS_MSCHAPV2_CHALLENGE_LEN);
        peer_challenge = challenge + 1 + EAP_TTLS_MSCHAPV2_CHALLENGE_LEN;
        if (mschap2_challenge_test != 2) { // ZTX: Challenge Failure 2 - Challenge Not Present
            if ( mschap2_challenge_test == 1 ) { // ZTX: Challenge Failure 1 - Challenge wrong
               int exchanged=0;
               int i,j;
               for ( i=0; i< EAP_TTLS_MSCHAPV2_CHALLENGE_LEN; i++) {
                   for ( j=i+1; j < EAP_TTLS_MSCHAPV2_CHALLENGE_LEN+1; j++) {
                       if (challenge[i] != challenge[j]) {                                  
                            u8 swap = challenge[i];
                            challenge[j] = challenge[i];
                            challenge[i] = swap; 
                            exchanged = 1;
                            break;  
                       }
        ...
        // 21/02/2008 | ZTX | END | Add for test flag of ms-chapv2 challenge test
	...
	*resp_len = pos - buf;
        ...
        /*if (no_data_test) {
           no_data_test_phase2_flag = 1;
           printf ("no data test phase2 entered!\n");
        }*/
	if (sm->workaround) {

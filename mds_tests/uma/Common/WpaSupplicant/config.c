...
static const struct parse_data ssid_fields[] = {
    ...
    { STR(EAPSimKC3) },
      ...
      * Session-Termination-Capability, PPAC, PPAQ, AAA-Session-ID
      */
	{ STR(servicetype) },
	{ STR(wimaxcapability) },
    	{ STR(session_termination_capability) },
	{ STR(ppac) },
...
void wpa_config_free_ssid(struct wpa_ssid *ssid)
{
    ...
    free(ssid->state);
    free(ssid->EAPSimSres1);
    free(ssid->EAPSimSres2);
    free(ssid->EAPSimSres3);
    free(ssid->EAPSimKC1);
    free(ssid->EAPSimKC2);
    free(ssid->EAPSimKC3);
	free(ssid->servicetype);
	free(ssid->wimaxcapability);
	free(ssid->session_termination_capability);
	free(ssid->ppac);
	free(ssid->aaa_session_id);
	free(ssid->ppaq);

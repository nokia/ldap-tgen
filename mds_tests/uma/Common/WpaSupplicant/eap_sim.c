...
static int eap_sim_gsm_auth(struct eap_sm *sm, struct eap_sim_data *data)
{
	 ...
	 * the order of pre-configured values. */
	{
        ...
        */
        int i;
        char hexa[5] = "0x";
        wpa_printf(MSG_DEBUG, "EAP-SIM: KC = %s %s %s ", config->EAPSimKC1, config->EAPSimKC2, config->EAPSimKC3);
        wpa_printf(MSG_DEBUG, "EAP-SIM: SRES = %s %s %s ", config->EAPSimSres1, config->EAPSimSres2, config->EAPSimSres3);
        if ((!config->EAPSimKC1)
            ||
            (!config->EAPSimKC2)
            ||
            ...
            ||
            (!config->EAPSimSres3)){
            wpa_printf( MSG_ERROR, "You must define EAPSimKC1, EAPSimKC2, EAPSimKC3, EAPSimSres1, EAPSimSres2 and EAPSimSres3 !!!");
            return -1;
        }
        
        for (i=0; i<KC_LEN; i++) {
            memcpy(&hexa[2], &(config->EAPSimKC1[i*2]),2);
            data->kc[0][i] = strtoul(hexa, (char **)NULL, 16);
            memcpy(&hexa[2], &(config->EAPSimKC2[i*2]),2);
            data->kc[1][i] = strtoul(hexa, (char **)NULL, 16);
            memcpy(&hexa[2], &(config->EAPSimKC3[i*2]),2);
            data->kc[2][i] = strtoul(hexa, (char **)NULL, 16);
        }
        for (i=0; i<SRES_LEN; i++) {
            memcpy(&hexa[2], &(config->EAPSimSres1[i*2]),2);
            data->sres[0][i] = strtoul(hexa, (char **)NULL, 16);
            memcpy(&hexa[2], &(config->EAPSimSres2[i*2]),2);
            data->sres[1][i] = strtoul(hexa, (char **)NULL, 16);
            memcpy(&hexa[2], &(config->EAPSimSres3[i*2]),2);
            data->sres[2][i] = strtoul(hexa, (char **)NULL, 16);
        }
        char ligne[80];
        for (i=0; i<3; i++) {
            sprintf(ligne, "KC %d : 0x%02x%02x%02x%02x%02x%02x%02x%02x", i, data->kc[i][0],
                           data->kc[i][1], data->kc[i][2], data->kc[i][3], data->kc[i][4],
                           data->kc[i][5], data->kc[i][6], data->kc[i][7]);
            wpa_printf( MSG_ERROR, "%s", ligne);
        }
        for (i=0; i<3; i++) {
            sprintf(ligne, "SRES %d : 0x%02x%02x%02x%02x", i, data->sres[i][0],
                           data->sres[i][1], data->sres[i][2], data->sres[i][3]);
            wpa_printf( MSG_ERROR, "%s", ligne);
        }

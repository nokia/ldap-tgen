 ...
 *
 * Version:	$Id: radius.c,v 1.108 2003/10/31 22:27:59 mcr Exp $
 *
...
static const char rcsid[] = "$Id: radius.c,v 1.108 2003/10/31 22:27:59 mcr Exp $";
		  ...
		  for (reply = packet->vps; reply; reply = reply->next) {
			  ...
			  case PW_TYPE_OCTETS:
				  ...
				  case FLAG_ENCRYPT_TUNNEL_PASSWORD:
					  rad_tunnel_pwencode((char *)reply->strvalue,
							      &(reply->length),
							      (const char *)secret,
							      (const char *)packet->vector);
					  break;
...
//		memcmp(packet->vector, calc_digest, sizeof(packet->vector)) ? 2 : 0;
	if ( memcmp(packet->vector, calc_digest, sizeof(packet->vector)) != 0 ) {
		packet->verified = 2;

#ifdef _DEBUG_INVALID_SIGNATURE
		fprintf(stderr,"Original packet:\n");
		for (i=0; i<100; i++)
			fprintf(stderr,"0x%2.2X ", original->data[i]);
		fprintf(stderr,"\n");

		fprintf(stderr,"Response packet:\n");
		for (i=0; i<100; i++)
			fprintf(stderr,"0x%2.2X ", packet->data[i]);
		fprintf(stderr,"\n");

		fprintf(stderr,"calc_digest:\n");
		for (i=0; i<16; i++)
			fprintf(stderr,"0x%2.2X ", calc_digest[i]);
		fprintf(stderr,"\n");

		fprintf(stderr,"Response vector:\n");
		for (i=0; i<16; i++)
			fprintf(stderr,"0x%2.2X ", packet->vector[i]);
		fprintf(stderr,"\n");
#endif
	} else
		packet->verified = 0;
...
/************************************************************************************/
{
	...
	if (librad_debug) {
		if ((hdr->code > 0) && (hdr->code < 52)) {
			fprintf(stderr,"rad_recv: %s packet from host %s:%d",
			       packet_codes[hdr->code],
			       ip_ntoa(host_ipaddr, packet->src_ipaddr), packet->src_port);
		} else {
			fprintf(stderr,"rad_recv: Packet from host %s:%d code=%d",	
			       ip_ntoa(host_ipaddr, packet->src_ipaddr), packet->src_port,
			       hdr->code);
		}
...
//EmA,11/12/03: TEMPORARY don't quit if MA is wrong
		...
		if ((vendorlen <= 0) &&
			...
			if (vendorcode != 0) {
				...
				if (vendorcode == VENDORPEC_USR) {
					ptr += 4;
					memcpy(&lvalue, ptr, 4);
					/*fprintf(stderr,"received USR %04x\n", ntohl(lvalue));*/
		...
		case PW_TYPE_IPADDR:
			...
			if (attr->type != PW_TYPE_IPADDR) {
				pair->lvalue = ntohl(lvalue);
				  ...
				  */
				pair->lvalue = lvalue;
				ip_ntoa((char*)pair->strvalue, pair->lvalue);
			}

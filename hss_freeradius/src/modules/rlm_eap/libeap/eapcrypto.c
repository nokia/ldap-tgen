 ...
 *
 * Version:     $Id: eapcrypto.c,v 1.1 2003/10/29 02:49:18 mcr Exp $
 *
...
void eapsim_dump_mk(struct eapsim_keys *ek)
{
	...
	fprintf(stderr,"   identity: (len=%d)", ek->identitylen);
	for (i = 0; i < ek->identitylen; i++) {
		fprintf(stderr,"%02x", ek->identity[i]);
	}

	fprintf(stderr,"\n   nonce_mt: ");
	for (i = 0; i < EAPSIM_NONCEMT_SIZE; i++) {
		fprintf(stderr,"%02x", ek->nonce_mt[i]);
	}

	for (k = 0; k<3; k++) {
		fprintf(stderr,"\n   rand%d: ", k);
		for (i = 0; i < EAPSIM_RAND_SIZE; i++) {
			fprintf(stderr,"%02x", ek->rand[k][i]);
		}
	}
	...
	for (k = 0; k<3; k++) {
		...
		for (i = 0; i < EAPSIM_SRES_SIZE; i++) {
			fprintf(stderr,"%02x", ek->sres[k][i]);
		}
	}
	...
	for (k = 0; k<3; k++) {
		...
		for (i = 0; i < EAPSIM_Kc_SIZE; i++) {
			fprintf(stderr,"%02x", ek->Kc[k][i]);
		}
	}

	fprintf(stderr,"\n   versionlist[%d]: ",ek->versionlistlen);
	for (i = 0; i < ek->versionlistlen; i++) {
		fprintf(stderr,"%02x", ek->versionlist[i]);
	}
	...
	for (i = 0; i < sizeof(ek->master_key); i++) {
		...
		fprintf(stderr,"%02x", ek->master_key[i]);
	}
	...
	for (i = 0; i < sizeof(ek->K_encr); i++) {
		...
		fprintf(stderr,"%02x", ek->K_encr[i]);
	}
	...
	for (i = 0; i < sizeof(ek->emsk); i++) {
		...
		fprintf(stderr,"%02x", ek->emsk[i]);
	}
	fprintf(stderr,"\n");
}

void eapsim_aesdecrypt(unsigned char * toDecrypt, int len, unsigned char* decrypted,
                              unsigned char iv[16], unsigned char Kencr[16])
{
    ...
    for (i = 0; i < len; i++) {
        fprintf(stderr,"%02x", toDecrypt[i]);
    }
    fprintf(stderr,"\n");
    ...
    fprintf(stderr,"Kencr ");
    for (i = 0; i < 16; i++) {
        fprintf(stderr,"%02x", Kencr[i]);
    }
    fprintf(stderr,"\n");
    */
    AES_set_decrypt_key(Kencr,128,&aes_ks1);
    
    AES_cbc_encrypt(toDecrypt,decrypted,
                    len,&aes_ks1,
                    iv,AES_DECRYPT);
    /*
...
void eapsim_aesencrypt(unsigned char * toEncrypt, int len, unsigned char* encrypted,
                              unsigned char iv[16], unsigned char Kencr[16])
{
    ...
    fprintf(stderr,"\n");
    */
    unsigned char inve[16];
    int i;
    for (i = 0; i < 16 ; i++) {
        inve[i] = iv[i];
    }
    AES_set_encrypt_key(Kencr,128,&aes_ks1);
    
    AES_cbc_encrypt(toEncrypt,encrypted,
                    len,&aes_ks1,
                    inve,AES_ENCRYPT);
    /*
    fprintf(stderr,"decrypted ");
...
void eapaka_calculate_keys(struct eapsim_keys *ek)
{
	SHA1_CTX context;
	uint8_t fk[160];
	unsigned char buf[256];
	...
	p = buf;
	memcpy(p, ek->identity, ek->identitylen);   p = p+ek->identitylen;
	memcpy(p, ek->ik, EAP_AKA_IK_LEN);       p = p+EAP_AKA_IK_LEN;
	memcpy(p, ek->ck, EAP_AKA_CK_LEN);       p = p+EAP_AKA_CK_LEN;
//	memcpy(p, ek->nonce_mt, sizeof(ek->nonce_mt)); p=p+sizeof(ek->nonce_mt);
//	memcpy(p, ek->versionlist, ek->versionlistlen);p=p+ek->versionlistlen;
//	memcpy(p, ek->versionselect, sizeof(ek->versionselect)); p=p+sizeof(ek->versionselect); 
	/* *p++ = ek->versionselect[1]; */
	...
	/* do the master key first *//* MK = SHA1(Identity|IK|CK) */
	SHA1Init(&context);
	SHA1Update(&context, buf, blen);
	SHA1Final(ek->master_key, &context);
...
void eapaka_dump_mk(struct eapsim_keys *ek)
{
	unsigned int i, j, k;
	...
	fprintf(stderr,"\n   ik: ");
	for (i = 0; i < EAP_AKA_IK_LEN; i++) {
		fprintf(stderr,"%02x", ek->ik[i]);
	}
	
	fprintf(stderr,"\n   ck: ");
	for (i = 0; i < EAP_AKA_CK_LEN; i++) {
		fprintf(stderr,"%02x", ek->ck[i]);
	}

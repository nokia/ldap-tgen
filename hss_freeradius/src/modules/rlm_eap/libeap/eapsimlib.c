 ...
 *
 * Version:     $Id: eapsimlib.c,v 1.1 2003/10/29 02:49:19 mcr Exp $
 *
...
static const char rcsid[] = "$Id: eapsimlib.c,v 1.1 2003/10/29 02:49:19 mcr Exp $";
...
int map_eapsim_basictypes(RADIUS_PACKET *r, EAP_PACKET *ep)
{
	 ...
	 *
     */
    /* EmA,09/08/2005: for notif & clienterror, need an EAP-Message even if no attribute inside it !!!
	if(encoded_size == 0)
	{
		return 0;
	}
    */
	...
	vp = pairfind(r->vps, ATTRIBUTE_EAP_TYPE);
	if(vp == NULL)
	{
	...
	if(vp == NULL)
	{
	       if (ep->type.type==PW_EAP_AKA)
	       {
		    encodedmsg[0] = eapaka_challenge;
	       }
		else
		{
...
int main(int argc, char *argv[])
{
	...
	while(!filedone) {
		...
		fprintf(stderr,"Unmapped to:\n");
		vp_printlist(stderr, req2->vps);
